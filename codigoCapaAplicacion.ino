...
const char ssid[] = "JOLUSE"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "1014JOLUSEfamilia"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "54.224.211.138"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
void sendSensorData(float temperatura, float humedad) {
  ...
  if (client.publish(MQTT_TOPIC_PUB, payload)) {
    Serial.println("Datos enviados correctamente a MQTT.");
  } else {
    Serial.println("Error al enviar los datos a MQTT.");
  }
}
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.print(data);
  // Verifica si el mensaje es una alerta crítica
  if (data.indexOf("ALERTA CRITICA: Activa") >= 0) {
    alert = "ALERTA CRITICA: Activa";
    digitalWrite(LED_PIN, HIGH);  // Enciende el LED (activo bajo)
    Serial.println("LED ENCENDIDO: ALERTA CRITICA ACTIVA");
  } else if (data.indexOf("ALERTA CRITICA: No activa") >= 0) {
    alert = "ALERTA CRITICA: No activa";
    digitalWrite(LED_PIN, LOW);   // Apaga el LED (activo bajo)
    Serial.println("LED APAGADO: ALERTA CRITICA NO ACTIVA");
...
void setup() {
  ...
  digitalWrite(LED_PIN, LOW); // Asegura que el LED esté apagado inicialmente (activo bajo)
}
